...
namespace HBL2::Editor
{
    ...
    ConsolePanel::ConsolePanel(const std::string& name, EditorPanelSystem* owner)
	{
		m_Owner = owner;
		Name = name;
	}
	...
	void ConsolePanel::OnOpen()
	{
	}

	void ConsolePanel::OnRender(float ts)
	{
        ...
        // Toolbar
        {
            ImGui::Checkbox("T", &m_View.ShowTrace); ImGui::SameLine();
            ImGui::Checkbox("I", &m_View.ShowInfo);  ImGui::SameLine();
            ImGui::Checkbox("W", &m_View.ShowWarn);  ImGui::SameLine();
            ImGui::Checkbox("E", &m_View.ShowError); ImGui::SameLine();
            ImGui::Checkbox("F", &m_View.ShowFatal); ImGui::SameLine();

            if (ImGui::Button("Clear"))
            {
                Console::Instance->ClearLog();
                m_SelectedSeq = UINT64_MAX;
            }

            ImGui::SameLine();
            ImGui::Checkbox("Auto-scroll", &m_View.AutoScroll);

            ImGui::SameLine();
            ImGui::Checkbox("Pause", &m_View.Pause);

            ImGui::SameLine();
            m_View.Filter.Draw("Search", 240.0f);
        ...
        const ConsoleSnapshot snap = m_View.Pause ? pausedSnap : Console::Instance->GetSnapshot();
        if (!m_View.Pause)
        {
            pausedSnap = snap;
        }
        ...
        for (uint32_t i = 0; i < snap.Count; ++i)
        {
            ...
            if (!PassesTypeFilter(m_View, msg.Type))
            {
                continue;
            }

            if (!PassesSearch(m_View, msg))
            {
                continue;
            }
        ...
        float effectiveDetailsH = m_DetailsOpen ? m_DetailsHeight : collapsedHeaderH;
        ...
        if (m_DetailsOpen)
        {
            m_DetailsHeight = std::max(minDetailsHeight, effectiveDetailsH);
        }
        ...
        while (clipper.Step())
        {
            for (int i = clipper.DisplayStart; i < clipper.DisplayEnd; ++i)
            {
                ...
                const bool isSelected = (seq == m_SelectedSeq);
        ...
        // Auto-scroll
        if (m_View.AutoScroll)
        {
            if (wasAtBottom && !m_View.Pause)
            {
        ...
        // Bottom collapsible details.
        {
            ...
            if (m_DetailsOpen)
            {
                ...
                {
                    MessageInfo sel{};
                    const bool ok = Console::Instance->TryReadMessageAtSequence(m_SelectedSeq, sel);

                    if (!ok)
                    {
                        ImGui::Text("Selected log (seq=%llu) is no longer available.", (unsigned long long)m_SelectedSeq);
                    }
                    else
                    {
                        if (!selectedVisible)
