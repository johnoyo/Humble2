...
namespace HBL2::Editor
{
	...
	StatsPanel::StatsPanel(const std::string& name, EditorPanelSystem* owner)
	{
		m_Owner = owner;
		Name = name;
	}
	...
	void StatsPanel::OnCreate()
	{
	}

	void StatsPanel::OnOpen()
	{
	}

	void StatsPanel::OnRender(float ts)
	{
		...
		ImGui::Text("Scene");
		ImGui::NewLine();
		ImGui::Text("Entities: %d", m_Owner->m_ActiveScene != nullptr ? m_Owner->m_ActiveScene->GetEntityCount() : 0);
		ImGui::NewLine();
	...
	void StatsPanel::OnClose()
	{
	}

	void StatsPanel::OnDestroy()
	{
	}
}
