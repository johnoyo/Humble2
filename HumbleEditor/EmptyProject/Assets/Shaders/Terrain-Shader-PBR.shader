...
const float baseStartHeights[3] = float[](
    0.01,
    0.15,
    0.65
);

const float baseBlends[3] = float[](
    0.0,
    0.0,
    0.25
);
...

{
    ...
    float heightPercent = InverseLerp(minHeight, maxHeight, v_Position.y);
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        terrainColor = terrainColor * vec3(1.0 - drawStrenght) + baseColors[i] * vec3(drawStrenght);
    }
