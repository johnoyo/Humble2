# Prompt: docs/generated/tests/design/name-resolution/visibility/
...
Produce the test bundle at `docs/generated/tests/design/name-resolution/visibility/`,
anchored to
...
covers where names are _visible within_ a scope chain;
...
2. A test file per **enforcement claim** in
   ...
   - **a `private`/`internal` decl is accessible from inside its
     ...
     permitted);
   - **a `private`/`internal` decl is _not_ accessible from outside
