...
5. `lowerCombinedTextureSamplers` is **skipped** for CUDA. The
   ...
   when `ArtifactDescUtil::isCpuLikeTarget(artifactDesc)` (line
   1761) holds, and CUDA is **not** CPU-like: a CUDA source or
...
| 10b | `stripAutoDiffDecorations` | [slang-ir-autodiff.cpp](../../../../source/slang/slang-ir-autodiff.cpp) | `!reqSet.autodiff` (`else` arm, line 1452) | Runs *instead of* row 10a. Removes the `Export` / `HLSLExport` / `KeepAlive` pins on the linked-in `[__AutoDiffBuiltin]` core-module types so row 12's DCE can drop them. |
...
inside `linkAndOptimizeIR`: once immediately after `linkIR` (line
1049) and once after specialization (line 1520). The flags
...
opcode in question; a gate can therefore be stale-*true* (the
construct was dead-code-eliminated after a scan, making the pass a
no-op walk) but never stale-*false*. The in-source comments at each
...
*do* contain autodiff IR and therefore never take the skip path.
...
attribute; the uniform/varying split *is* the selection rule.
...
but it only scans the entry point's *own* blocks. A terminating
...
entry-point *result* before the parameter loop runs. Diagnostics
...
while the *signature* is CUDA-clean.
