...
namespace HBL2
{
	ShaderUtilitiesOld* ShaderUtilitiesOld::s_Instance = nullptr;
	...
	ShaderUtilitiesOld& ShaderUtilitiesOld::Get()
	{
		HBL2_CORE_ASSERT(s_Instance != nullptr, "ShaderUtilitiesOld::s_Instance is null! Call ShaderUtilitiesOld::Initialize before use.");
		return *s_Instance;
	}

	void ShaderUtilitiesOld::Initialize()
	{
		HBL2_CORE_ASSERT(s_Instance == nullptr, "ShaderUtilitiesOld::s_Instance is not null! ShaderUtilitiesOld::Initialize has been called twice.");
		s_Instance = new ShaderUtilitiesOld;
	}

	void ShaderUtilitiesOld::Shutdown()
	{
		HBL2_CORE_ASSERT(s_Instance != nullptr, "ShaderUtilitiesOld::s_Instance is null!");

		delete s_Instance;
		s_Instance = nullptr;
	}

	ShaderUtilitiesOld::ShaderUtilitiesOld()
	{
		m_Reservation = Allocator::Arena.Reserve("ShaderUtilitiesOldPool", 16_MB);
	...
	std::string ShaderUtilitiesOld::ReadFile(const std::string& filepath)
	{
		std::string result;
	...
	std::vector<uint32_t> ShaderUtilitiesOld::Compile(const std::string& shaderFilePath, const std::string& shaderSource, ShaderStage stage, bool forceRecompile)
	{
		HBL2_FUNC_PROFILE();
	...
	std::vector<std::vector<uint32_t>> ShaderUtilitiesOld::Compile(const std::string& shaderFilePath, bool forceRecompile)
	{
		HBL2_FUNC_PROFILE();
	...
	void ShaderUtilitiesOld::LoadBuiltInShaders()
	{
	...
	void ShaderUtilitiesOld::DeleteBuiltInShaders()
	{
	...
	void ShaderUtilitiesOld::LoadBuiltInMaterials()
	{
	...
	void ShaderUtilitiesOld::DeleteBuiltInMaterials()
	{
		AssetManager::Instance->DeleteAsset(LitMaterialAsset);
	}

	void ShaderUtilitiesOld::CreateShaderMetadataFile(Handle<Asset> handle, uint32_t shaderType)
	{
	...
	void ShaderUtilitiesOld::UpdateShaderVariantMetadataFile(UUID shaderUUID, const ShaderDescriptor::RenderPipeline::PackedVariant& newVariant)
	{
	...
	void ShaderUtilitiesOld::CreateMaterialMetadataFile(Handle<Asset> handle, uint32_t materialType, bool autoImported)
	{
		Asset* asset = AssetManager::Instance->GetAssetMetadata(handle);
	...
	void ShaderUtilitiesOld::CreateMaterialAssetFile(Handle<Asset> handle, const MaterialDataDescriptor&& desc)
	{
	...
	ReflectionData ShaderUtilitiesOld::Reflect(const Span<uint32_t>& vertexShaderData, const Span<uint32_t>& fragmentShaderData, const Span<uint32_t>& computeShaderData)
	{
	...
	const char* ShaderUtilitiesOld::GetCacheDirectory(GraphicsAPI target)
	{
		switch (target)
		{
		case GraphicsAPI::OPENGL:
	...
	void ShaderUtilitiesOld::CreateCacheDirectoryIfNeeded(GraphicsAPI target)
	{
	...
	const char* ShaderUtilitiesOld::GLShaderStageCachedVulkanFileExtension(ShaderStage stage)
	{
		switch (stage)
		{
	...
	const char* ShaderUtilitiesOld::GLShaderStageCachedOpenGLFileExtension(ShaderStage stage)
	{
		switch (stage)
		{
		case ShaderStage::VERTEX:
	...
	shaderc_shader_kind ShaderUtilitiesOld::GLShaderStageToShaderC(ShaderStage stage)
	{
		switch (stage)
		{
		case ShaderStage::VERTEX:
	...
	const char* ShaderUtilitiesOld::GLShaderStageToString(ShaderStage stage)
	{
		switch (stage)
		{
		case ShaderStage::VERTEX:
