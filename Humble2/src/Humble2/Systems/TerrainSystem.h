...
namespace HBL2
{
	...
	private:
		...
		std::vector<float> GenerateFallofMap(const Component::Terrain& terrain, int32_t size);
		std::vector<float> GenerateNoiseMap(const Component::Terrain& terrain, int32_t size, const glm::vec2& center);
		std::vector<float> GetChunkNoiseMap(const std::vector<float>& src, int32_t srcW, int32_t srcH, int32_t chunkQuads, int32_t chunkX, int32_t chunkY);
		...
		void UpdateVisibleChunks(Entity terrainEntity, Component::Terrain& terrain, Component::AnimationCurve& curve, const Component::Transform& viewer);
