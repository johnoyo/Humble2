...
namespace HBL2
{
	void MeshRendererSystem::OnCreate()
	{
		...
		Context::ActiveScene->GetRegistry()
			.group<Component::Mesh>(entt::get<Component::Transform>)
			.each([&](Component::Mesh& mesh, Component::Transform& transform)
			{
				if (mesh.Enabled)
				{
					transform.QRotation = glm::quat(transform.Rotation);
					transform.Matrix = glm::translate(glm::mat4(1.0f), transform.Position) * glm::toMat4(transform.QRotation) * glm::scale(glm::mat4(1.0f), transform.Scale);
